...
def main():
    ...
    results_dir = Path("results/vsm-lm-v5")
    ...
    banner(f"VSM-LM v5 — Spiral Bias VSM 1B TOKENS ({N_STEPS} STEPS)")

    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/pythia-160m-deduped")

    tokens_total = N_STEPS * TOKENS_PER_STEP
    print(f"  Device: {device}")
    print(f"  Architecture: VSM-LM-v5 (v4.1 + spiral bias α={ALPHA})")
    ...
    print(f"  Strides: {STRIDES} (4 scales, progressive reallocation, α={ALPHA})")
    ...
    summary = {
        ...
        "elapsed_s": elapsed,
        "architecture": "VSM-LM-v5 (v4.1 + spiral bias α=1.18, 1B tokens)",
